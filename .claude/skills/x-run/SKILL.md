...
Claude がブラウザで以下まで操作する。最後の「スケジュール」ボタンはユーザーが押す。

```
📅 操作手順:
1. @moyuchi_cc でログイン中であることを確認 ← 必須
2. x.com/compose/post を開く
3. 草稿のテキストを入力
4. 📅 アイコン（スケジュール）をクリック
5. 推奨日時: {推奨投稿時間} を設定
6. ← ここでユーザーに確認を求めて停止 →
7. ユーザーが「スケジュール」ボタンを押す
```

**B. Buffer / Hootsuite 等のスケジューラーを使う**
```
📅 推奨投稿時間: {推奨投稿時間}
   草稿をコピーしてスケジューラーに貼り付けてください
```

送信・スケジュール登録は必ずユーザーが行う。
